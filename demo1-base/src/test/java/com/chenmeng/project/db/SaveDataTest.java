...
class SaveDataTest {
    ...
    /**
     * 批量插入数据（interface_info表）
     */
    @Test
    void saveInterfaceInfoData() {
        List<InterfaceInfoDO> list = new ArrayList<>();
        for (int i = 0; i < 999; i++) {
            InterfaceInfoDO bean = new InterfaceInfoDO();
            bean.setName("接口名称");
            bean.setDescription("接口描述");
            bean.setUrl("接口地址");
            bean.setRequestParams("请求参数");
            bean.setRequestHeader("请求头");
            bean.setResponseHeader("响应头");
            bean.setMethod("GET");
            bean.setStatus(0);
            if (i % 1000 == 0) {
                interfaceInfoService.saveBatch(list);
                list.clear();
            }
        }
        if (!list.isEmpty()) {
        ...
        System.out.println("count() = " + interfaceInfoService.count());
    }
}